...
## Decisões operacionais (2026-08-25)

- **gRPC/`cluster-agent` desabilitado por ora** — o time decidiu não usar o caminho de agente discando por enquanto (talvez no futuro); o único caminho ativo é IAM Role/STS via `internal/rolecluster`. O código do `cluster-agent`/`internal/transport`'s `AgentService` continua no repo (testado, funcional), só não está sendo operado. Consequência direta: **mTLS (emissão/rotação de certificado por cluster) deixa de ser necessário nesse modo** — mTLS existe especificamente para a conexão `cluster-agent`↔hub e o forwarding `InternalService` entre réplicas (`internal/registry`), ambos exclusivos do caminho gRPC. Sem gRPC, `internal/registry`/Redis multi-réplica também fica sem função (clusters via Role já não precisam disso — ver "Registro compartilhado entre réplicas do hub" acima).
- **TLS do endpoint MCP**: será terminado no ELB/ALB na frente do `hub-server` no EKS, não no processo Go — mesma camada que já precisa de sticky session por cookie (ver "Registro compartilhado..." acima). O `hub-server` continua servindo HTTP puro internamente, atrás dessa borda.
- **Identidade do chamador (AD) — simulada, não real ainda**: `internal/agentauth` agora carrega grupos AD simulados por token (`ad_groups` no YAML de `--agent-scopes-config`), resolvidos por sessão MCP junto com o escopo de conta AWS — substitui o `--test-caller-groups` global por sessão quando um token resolve. Isso exercita RBAC de tier e de visibilidade de tool ponta-a-ponta sem SSO real, mas **ainda não é identidade real** — é só simulação pra teste. Ver `docs/inventory-api-decision.md` — a integração real de identidade (e de inventário) fica pendente de definição com o time antes do ambiente da empresa.

Gaps conhecidos, ainda não resolvidos:

- **Identidade do chamador**: simulada por token (ver acima), não propagada de verdade — pendente de como o `wb-ce-agent` (ou outro chamador) vai carregar a identidade real (SSO/OIDC) na chamada MCP.
- **API de inventário de clusters**: contrato assumido em `internal/inventory.HTTPClient`, não confirmado — ver `docs/inventory-api-decision.md` para o gap concreto encontrado contra o mock local (`goldenbridge`).
- **`ClusterEnv` (prod/nonprod) por cluster** vem de uma flag CSV (`--prod-clusters`) no hub-server, não de um inventário — suficiente pros testes locais. `goldenbridge` já tem um campo `env` por conta/recurso que poderia substituir isso, a confirmar junto com o item acima.
- **Bug de validação conhecido, não corrigido**: `core/crashloopbackoff` (rungs "scale down/up" e "rollout restart") e `core/oomkilled.Recheck` verificam `ReadyReplicas >= want` uma única vez — sem readiness probe no pod, um container recém-criado conta como Ready no instante em que sobe, antes de voltar a crashar. Pode reportar `resolved: true`/aprovação liberada quando o problema continua. Reproduzido ao vivo em 2026-08-25 contra Floci. `--execution-enabled` fica desligado por padrão até isso ser corrigido, pra limitar o dano desse falso-positivo.
